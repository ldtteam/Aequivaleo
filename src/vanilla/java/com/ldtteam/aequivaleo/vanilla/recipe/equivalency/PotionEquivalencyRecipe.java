...
public class PotionEquivalencyRecipe extends BaseEquivalencyRecipe implements IPotionEquivalencyRecipe {
    ...
    public PotionEquivalencyRecipe(
        ...
        super(
                ...
                Stream.of(input, reagent).map(ItemStack::getCraftingRemainingItem).filter(Predicate.not(ItemStack::isEmpty))
                        .map(ICompoundContainerFactoryManager.getInstance()::wrapInContainer)
                        .map(SimpleIngredientBuilder::simple)
